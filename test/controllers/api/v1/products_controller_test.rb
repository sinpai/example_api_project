...
class Api::V1::ProductsControllerTest < ActionDispatch::IntegrationTest
  setup do
    user = User.create(email: "1@email.com", password: 'qweasd')
    @product = user.products.create!(title: 'new', price: 10, published: 1)
  end
  ...
  test "should forbid update product" do
    patch api_v1_product_url(@product),
          params: { product: { title: @product.title } },
          headers: { Authorization: JsonWebToken.encode(user_id: User.create(email: "3@email.com", password: 'qweasd').id ) },
          as: :json
    assert_response :forbidden
  end
